...
while not depsonly:
    try:
        request = urllib.request.Request("https://api.github.com/users/FuryDragons/repos?page=%d" % page)
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/furydragons_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print ('FuryDragons/%s already exists' % (repo_name))
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "furydragons", "name": repo_name, "revision": "FDS-N" })
    ...
    f = open('.repo/local_manifests/furydragons_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "furydragons", "name": "FuryDragons/%s" % repo_name, "revision": "FDS-N" })
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/furydragons.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'FDS-N'}])
...
print("Repository for %s not found in the FuryDragons Github repository list. If this is in error, you may need to manually add it to your local_manifests/furydragons_manifest.xml." % device)
